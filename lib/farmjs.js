 ...
 */
Router.prototype.spin = function(name, callback) {
    ...
    return self._resolveApp(self.logger, name, function(err, app) {
        ...
        return self._spawn(name, app.spawn, app.basename, callback);
    });
};
 ...
 */
Router.prototype._spawn = function(name, spawnOptions, basename, callback) {
    var self = this;
    ...
    self.logger.log('Spinning', spawnOptions.command, spawnOptions.args);
    ...
    options.env.FARMJS_APP = basename;
    options.env.FARMJS_APP_FULLNAME = name;
    options.env.FARMJS_INSTANCE = self.options.instance;

    // spawn app
    return self.spinner.start(options, callback, self.logger);
};
