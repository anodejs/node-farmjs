...
function Farmjs(opts) {
	...
	if (!opts.instances) opts.instances = {};
	...
	//
	// instances is a hash that maps instance id to { host: HOST, port:PORT }
	// objects used to implement $bcast and $inst hints.
	//
...
Farmjs.prototype._handlers = {
	...
	inst: function(req, res, next) {
		...
		var address = self.instances[inst];
		if (!address) return res.error(400, new Error("unknown instance '" + inst));
