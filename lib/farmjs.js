...
function Router(opts) {
    ...
    self.metrics = metrics(metricsOptions);
...
Router.prototype._handlers = {
    ...
    context: function(req, res, next) {
        ...
        res.end = function() {
            ...
            var duration = req.timer.stop(self._getMetricName(req, "duration"));
            // and without
            self.metrics.write("proxy.duration", duration);
    ...
    spawn: function(req, res, next) {
        ...
        self.spinner.start(options, function(err, port) {
            if (err) {
                ...
                return res.error(500, new Error("unable to spawn app " + req.app.name + ". " + err.toString()));
            }
...
Router.prototype._parseURL = function(req) {
    ...
    else if (req.headers[exports.HEADERS.APP_FULLNAME]) {
        req.logger.log('app defined by ' + exports.HEADERS.APP_FULLNAME);
        url.app = req.headers[exports.HEADERS.APP_FULLNAME];
    }
    else {
