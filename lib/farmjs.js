...
Farmjs.prototype._handlers = {
	...
	context: function(req, res, next) {
		...
		res.error = function(status, obj) {
			...
			req.logger.error(status + " " + JSON.stringify(obj));
	...
	spawn: function(req, res, next) {
		...
		//
		// If spawn.script is not defined, how can we spawn?
		//

		if (!req.app.spawn.script) {
			return res.error(500, "No spawn script defined for app " + req.app.name);
		}
		...
		if (!req.app.spawn.command) req.app.spawn.command = 'node';
		...
		self.spinner.spin(req.app.spawn.script, req.app.spawn, function(err, port) {
