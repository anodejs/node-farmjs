...
Resolver.prototype.addRootDomain = function (rootDomain) {
    this.rootDomains.push(rootDomain);
};
...
Resolver.prototype.resolve = function (hostname, pathname) {
    ...
    function resolveAsRootDomain() {
        ...
        for (var i in self.rootDomains) {
            appName = subdomain(hostname, self.rootDomains[i]);
            if (appName) break;
        }

        if (appName === 'www') {
            appName = null; // www is not a subdomain
        }

        return appName ? { app: appName, pathname: pathname } : null;
    }

    function resolveAsPath(req) {
        ...
        return (appName) ? { app: appName, pathname: targetUrl } : null;
    }
